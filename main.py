...
if __name__ == "__main__":
    ...
    if params["mode"] == 0:
        ...
        cur_map_path = segment_map(
            cur_map_path, cur_new_map_path, contour=0
        )  # save the final prediction prob array space
        ...
        if np.max(data) <= params["contour"]:
            print(
                "!!!Warning!!!Please provide contour level lower than maximum density"
                " value to run!"
            )
            # exit()
        ...
        if params["prediction_only"]:
            print(
                "Our prediction results are saved in %s with mrc format for"
                " visualization check." % save_path_2nd_stage
            )
            exit()
