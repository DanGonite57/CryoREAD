...
class Points(object):
    ...
    def Merge_point(self, mrc, point_path):
        ...
        self.stock, self.member = acc_merge_point(
            tmp_Ncd,
            tmp_dens,
            dmin,
            rv_range,
        ...
        self.merged_cd_dens = np.zeros(
            [Nmerge, 4]
        )  # first 3 column is the coordinates, the last 1 is the density
    ...
    def normalize(self):
        self.merged_cd_dens[:, 3] = (
            self.merged_cd_dens[:, 3] - np.min(self.merged_data[:, 4])
        ) / (
            np.max(self.merged_data[:, 4]) - np.min(self.merged_data[:, 4])
        ) + 0.01  # for 0 happen, which will raise problems in edge density calculation
        # self.min_dens=np.min(self.merged_data[:,4])
        # self.max_dens=np.max(self.merged_data[:,4])
        self.merged_data[:, 4] = (
            self.merged_data[:, 4] - np.min(self.merged_data[:, 4])
        ) / (np.max(self.merged_data[:, 4]) - np.min(self.merged_data[:, 4]))
    ...
    def clean_isolate_data(self, graph):
        ...
        for ii in range(self.Nori):
            ...
            if merged_id == -1:
                ...
                continue
            self.mask[ii] = tmp_mask[
                merged_id
            ]  # mark this original point that it is in edge or not
        print(
            "We have %d/%d isolated points"
            % (len(np.argwhere(self.mask == 0)), self.Nori)
        )
