...
class MRC(object):
    ...
    def read_mrc(self, file_name=None, contour=0):
        ...
        with mrcfile.open(self.file_name, permissive=True) as mrc:
            ...
            self.nx, self.ny, self.nz = (
                ...
                mrc.header.ny,
                mrc.header.nz,
            )  # number of columns/rows/sections in 3 dimensions
            ...
            self.mx, self.my, self.mz = (
                ...
                mrc.header.my,
                mrc.header.mz,
            )  # sampling along X,Y,Z axis of unit cell
            ...
            self.mapc, self.mapr, self.maps = (
                ...
                mrc.header.mapr,
                mrc.header.maps,
            )  # axis corresponds to column/row/section, 1 represents X axis,2 is Y, 3 is Z
    ...
    def print_info(self):
        ...
        print(
            "location of first column/row/section in unit cell:%d,%d,%d"
            % (self.nxstart, self.nystart, self.nzstart)
        )
        print(
            "sampling along X,Y,Z axis of unit cell:%d,%d,%d"
            % (self.mx, self.my, self.mz)
        )
        print(
            "cell dimensions in angstroms:%f,%f,%f" % (self.xlen, self.ylen, self.zlen)
        )
        print("Cell angles in degree:%d,%d,%d" % (self.alpha, self.beta, self.gamma))
        print("axis mode:(%d,%d,%d)" % (self.mapc, self.mapr, self.maps))
        print(
            "density statistics:min(%f),max(%f),mean(%f)"
            % (self.dmin, self.dmax, self.dmean)
        )
    ...
    def reset_main_chain_prob(self, chain_prob, threshold):
        ...
        print(self.shape)
        print(
            "x dim now %d,ydim now %d, zdim now %d" % (self.xdim, self.ydim, self.zdim)
        )
        ...
        print(
            "after normalizing dens min %.4f max %.4f"
    ...
    def upsampling_chain_prob(self, chain_prob, threshold=0.01):
        ...
        main_chain_prob[main_chain_prob <= threshold] = (
            0  # only keep our main chain prediction
        )
        ...
        self.shape = self.dens.shape
        print(
            "after normalizing chain dens min %.4f max %.4f"
    ...
    def upsampling_sugar_prob(self, main_chain_prob, threshold=0.01, filter_array=None):
        ...
        self.shape = self.dens.shape
        print(
            "after normalizing sugar dens min %.4f max %.4f"
    ...
    def upsampling_pho_prob(self, main_chain_prob, threshold=0.01, filter_array=None):
        ...
        print(
            "after normalizing pho dens min %.4f max %.4f"
            % (np.min(self.pho_dens), np.max(self.pho_dens))
        )
        return True

    def upsampling_specify_prob(
        self, specify_name, main_chain_prob, threshold=0.01, filter_array=None
    ):
        # main_chain_prob = nuc_prob[2]
        main_chain_prob[main_chain_prob <= threshold] = (
            0  # only keep our main chain prediction
        ...
        print(
            "in total chain useful percentage %.6f"
                ...
                for k in range(self.shape[2]):
                    ...
                    if chain_prob[0, i, j, k] >= threshold:
                        ...
                        density[i, j, k] = chain_prob[
                            0, i, j, k
                        ]  # np.sum(nuc_prob[:4,i,j,k]) #only keep the prob of bases
    ...
    def general_mean_shift(self, density, point, mean_shift_path, constriant=False):
        ...
        )  # self.params['g']  # bandwidth of Gaussian distribution
        gstep = self.widthx
        ...
        if not constriant:
            point.cd, point.dens = carry_shift(
                point.cd, cnt, fmaxd, fsiv, tmp_xdim, tmp_ydim, tmp_zdim, tmp_dens
            )
            ...
            point.cd, point.dens = carry_shift_limit(
                point.cd,
                cnt,
                fmaxd,
                fsiv,
