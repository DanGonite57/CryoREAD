...
class Main_Graph(object):
    ...
    def setup_graph(self, point, tmp_data_path):
        ...
        for i in range(tmp_Ne):
            ...
            if result not in self.adj:
                ...
                if (
                    i_index * self.Nnode + j_index not in self.adj
                ):  # avoid double adding
                    self.Ne += 1
        ...
        if not os.path.exists(edge_info_path):
            ...
            if len(all_edge_info) != self.Ne or (
                all_edge_info[0, 1] == 0 and all_edge_info[0, 2] == 0
            ):
    ...
    def set_edge_dens(self, tmp_data_path, mrc, point, density):
        ...
        print(
            "edge density min %.2f max %.2f mean %.2f"
            % (min(tmp_dens), max(tmp_dens), np.mean(tmp_dens))
        )
    ...
    def calcu_local_label(self, point, d2cut):
        ...
        tmp_local_label, tmp_count = acc_local_mst(
            cid,
            tmp_Nnode,
            tmp_Ne,
            tmp_id1,
    ...
    def config_edge_info(self, point, tmp_data_path):
        ...
        with open(node_path, "w") as file:
            for i in range(self.Nnode):
                ...
                for k in range(4):
                    line = (
                        line + str(point.merged_cd_dens[i, k]) + " "
                    )  # x,y,z+density record
        ...
        if not os.path.exists(edge_path):
            np.savetxt(
                edge_path, tmp_edge_data
            )  # If not, the corruption may happen when some one reading on paralleled situation.
    ...
    def assign_node_prob(self, All_Prob_Array, point):
        ...
        for i in range(self.Nnode):
            ...
            self.node[i].atom_prob = np.sum(
                All_Prob_Array[Name_list[0]][:, x1:x2, y1:y2, z1:z2], axis=(1, 2, 3)
            )
            self.node[i].nuc_prob = np.sum(
                All_Prob_Array[Name_list[1]][:, x1:x2, y1:y2, z1:z2], axis=(1, 2, 3)
            )
            self.node[i].chain_prob = np.sum(
                All_Prob_Array[Name_list[2]][:, x1:x2, y1:y2, z1:z2], axis=(1, 2, 3)
            )
            self.node[i].base_prob = np.sum(
                All_Prob_Array[Name_list[3]][:, x1:x2, y1:y2, z1:z2], axis=(1, 2, 3)
            )
    ...
    def build_subgraph(self, connect_cid):
        ...
        for select_id in unique_id:
            ...
            if len(node_id_list) < 10:
                print(
                    "cid %d only included %d nodes,skip!"
...
def Build_connect_dens_dict(adj_record, density_record, save_path):
    ...
    for i in range(length):
        ...
        if search_index not in search_dict.keys():
            ...
            search_dict[int(search_index)] = max(
                density_record[i], search_dict[search_index]
            )
