from graph.LDP_ops import (
    permute_point_coord_to_global_coord,
    permute_global_coord_to_point_coord,
)
...
def read_refer_dict(pdb_path):
    ...
    bpp_cos_values = calculate_cosine_angle(
        base_center_location, backbone_location_list[0], backbone_location_list[-1]
    )
    bp1_distance = calculate_point_distance(
        base_center_location, backbone_location_list[0]
    )
    p1_p2_distance = calculate_point_distance(
        backbone_location_list[0], backbone_location_list[-1]
    )
    p1_p2_vector = backbone_location_list[-1] - backbone_location_list[0]
    orthognal_location = (
        backbone_location_list[0]
        + p1_p2_vector * (bpp_cos_values * bp1_distance) / p1_p2_distance
    )
    ...
    return (
        ...
        backbone_location_list,
        orthognal_ratio,
        p1p2_to_base_direction,
        p1_to_base_direction,
    )
...
def read_refer_dict_sugar(pdb_path):
    ...
    bss_cos_values = calculate_cosine_angle(
        base_center_location, sugar_location_list, next_sugar_location_list
    )
    bs1_distance = calculate_point_distance(base_center_location, sugar_location_list)
    s1_s2_distance = calculate_point_distance(
        sugar_location_list, next_sugar_location_list
    )
    orthognal_location = (
        sugar_location_list
        + p1_p2_vector * (bss_cos_values * bs1_distance) / s1_s2_distance
    )
    ...
    return (
        ...
        [sugar_location_list, next_sugar_location_list],
        orthognal_ratio,
        s1s2_to_base_direction,
        s1_to_base_direction,
    )
...
def build_base_atomic_nuc_rigid(
    input_pho_position, next_pho_location, cur_base_location, nuc_type, map_info_list
):
    point_pho_location = permute_global_coord_to_point_coord(
        input_pho_position, map_info_list
    )
    revert_back_pho_location = permute_point_coord_to_global_coord(
        point_pho_location, map_info_list
    )
    for k in range(3):
        assert abs(revert_back_pho_location[k] - input_pho_position[k]) <= 0.01
    point_next_pho_location = permute_global_coord_to_point_coord(
        next_pho_location, map_info_list
    )
    point_pho_location = np.array(point_pho_location)
    point_next_pho_location = np.array(point_next_pho_location)
    point_base_location = permute_global_coord_to_point_coord(
        cur_base_location, map_info_list
    )
    cos_base_p1_p2 = calculate_cosine_angle(
        point_base_location, point_pho_location, point_next_pho_location
    )
    base_p1_distance = calculate_point_distance(point_base_location, point_pho_location)
    p1_p2_distance = calculate_point_distance(
        point_pho_location, point_next_pho_location
    )
    orthognal_location = (
        point_pho_location
        + (point_next_pho_location - point_pho_location)
        * (cos_base_p1_p2 * base_p1_distance)
        / p1_p2_distance
    )
    ...
    (
        all_location_dict,
        standard_backbone_location_list,
        standard_orthognal_ratio,
        standard_p1p2_to_base_direction,
        standard_p1_base_direction,
    ) = current_standard_info
    p1_p2_select_orthognal_location = point_pho_location + standard_orthognal_ratio * (
        point_next_pho_location - point_pho_location
    )
    base_direction_adjust_ratio = calculate_point_distance(
        [0, 0, 0], standard_p1p2_to_base_direction
    ) / calculate_point_distance([0, 0, 0], p1p2_base_orthognal_direction)
    p1_p2_adjust_base_location = (
        p1_p2_select_orthognal_location
        + p1p2_base_orthognal_direction * base_direction_adjust_ratio
    )
    ...
    standard_align_vector.append(
        standard_p1_base_direction + standard_backbone_location_list[0]
    )
    for k in range(len(standard_align_vector)):
        standard_align_vector[k] = (
            standard_align_vector[k] - standard_backbone_location_list[0]
        )
    ...
    point_final_o_location = point_pho_location + (
        divide_number - 1
    ) / divide_number * (point_next_pho_location - point_pho_location)
    ...
    rotation, translation, err = rigid_transform_3D(
        standard_align_vector, current_bpp_vector
    )
    all_locations = np.array(list(all_location_dict.values()))
    output_locations = np.matmul(all_locations, rotation.T) + translation.reshape(
        [1, 3]
    )
    ...
    output_locations = [
        permute_point_coord_to_global_coord(output_locations[k], map_info_list)
        for k in range(len(output_locations))
    ]
    return search_key_list, output_locations


def build_base_atomic_nuc_rigid_sugar(
    input_pho_position, next_pho_location, cur_base_location, nuc_type, map_info_list
):
    point_pho_location = permute_global_coord_to_point_coord(
        input_pho_position, map_info_list
    ...
    (
        ...
        standard_backbone_location_list,
        standard_orthognal_ratio,
        standard_s1s2_to_base_direction,
        standard_s1_base_direction,
    ) = current_standard_info

    s1_s2_select_orthognal_location = point_pho_location + standard_orthognal_ratio * (
        point_next_pho_location - point_pho_location
    )
    base_direction_adjust_ratio = calculate_point_distance(
        [0, 0, 0], standard_s1s2_to_base_direction
    ) / calculate_point_distance([0, 0, 0], p1p2_base_orthognal_direction)
    s1_s2_adjust_base_location = (
        s1_s2_select_orthognal_location
        + p1p2_base_orthognal_direction * base_direction_adjust_ratio
    )
    ...
    standard_align_vector.append(standard_backbone_location_list[-1])
    standard_align_vector.append(
        standard_s1_base_direction + standard_backbone_location_list[0]
    )
    for k in range(len(standard_align_vector)):
        standard_align_vector[k] = (
            standard_align_vector[k] - standard_backbone_location_list[0]
