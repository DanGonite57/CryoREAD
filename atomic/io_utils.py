...
from atomic.geo_atomic_modeling import (
    build_base_atomic_nuc_rigid,
    build_base_atomic_nuc_rigid_sugar,
)
...
def Write_Atomic_Fraginfo_cif(
    name, frag_info_list, refer_base_location, save_path, DNA_Label, map_info_list
):
    Natm = 1
    pdb_save_path = save_path[:-4] + ".pdb"
    with open(pdb_save_path, "w") as file:
        file.write("")
    refer_sugar_base_location = define_base_location_list(
        frag_info_list, refer_base_location
    )
        ...
        for k in range(len(Sugar_Only_Frag_List)):
            ...
            if current_seq_index in Sugar_Refer_Dict:
                (
                    ...
                    nuc_type,
                    avg_score,
                ) = Sugar_Refer_Dict[current_seq_index]
                if k + 1 < len(Sugar_Only_Frag_List):
                    _, next_seq_index, _, next_sugar_position, _, _ = (
                        Sugar_Only_Frag_List[k + 1]
                    )
                    if next_seq_index in Sugar_Refer_Dict:
                        _, next_seq_index, _, next_pho_position, _, _ = (
                            Sugar_Refer_Dict[next_seq_index]
                        )
                    ...
                    _, prev_seq_index, _, prev_sugar_position, _, _ = (
                        Sugar_Only_Frag_List[k - 1]
                    )
                    if prev_seq_index in Sugar_Refer_Dict:
                        _, prev_seq_index, _, prev_pho_position, _, _ = (
                            Sugar_Refer_Dict[prev_seq_index]
                        )
                ...
                if k >= 1 and k + 1 < len(Sugar_Only_Frag_List):
                    ...
                    if (
                        prev_seq_index in Sugar_Refer_Dict
                        and next_seq_index in Sugar_Refer_Dict
                    ):
                        _, prev_seq_index, _, prev_pho_position, _, _ = (
                            Sugar_Refer_Dict[prev_seq_index]
                        )
            ...
            if use_pho_model > 0:
                ...
                atom_list, coordinate_list = build_base_atomic_nuc_rigid(
                    cur_pho_position,
                    next_pho_position,
                    cur_base_location,
                    nuc_type,
                ...
                atom_list, coordinate_list = build_base_atomic_nuc_rigid_sugar(
                    cur_sugar_position,
                    next_sugar_position,
                    cur_base_location,
                    nuc_type,
            ...
            for k in range(len(atom_list)):
                ...
                Natm += 1
        print(
            "we have %d/%d atomic build upon pho atoms"
            % (count_use_pho, len(Sugar_Only_Frag_List))
        )
