...
def acc_new_adj_matrix(Nori, merged_data, adj, origrid, Nnode):
    ...
    for ii in range(Nori):
        ...
        for jj in range(ii + 1, Nori):
            ...
            if (
                merged_id2 == -1 or merged_id1 == merged_id2
            ):  # To avoid that point with stock=0, very lower density, however, never be combined to others
                continue
...
@jit(nogil=True, nopython=True)
def acc_edge_prob(
    Ne, id1, id2, merged_cd_dens, dens, edge_d, fsiv, fmaxd, xdim, ydim, zdim, mrc_dense
):
    if True:
        for ii in range(Ne):
            if ii % 1000 == 0:
            ...
            dens[ii] = (
                edge_d[ii] / MinDens
            )  # prob bigger, distance smaller is our favorite choice
    return dens


@jit(nogil=True, nopython=True)
def acc_edge_density(
    Ne, id1, id2, merged_cd_dens, dens, edge_d, fsiv, fmaxd, xdim, ydim, zdim, mrc_dense
):
    if True:
        for ii in range(Ne):
            ...
            for kk in range(3):
                vec[kk] = merged_cd_dens[v2, kk] - merged_cd_dens[v1, kk]
            MinDens = merged_cd_dens[
                v1, 3
            ]  # Notice 09/25/2018, important update, we need to get the unnormalized density here, actually we did
            cd1 = np.zeros(3)
            for jj in range(11):
                for kk in range(3):
                    cd1[kk] = merged_cd_dens[v1, kk] + vec[kk] * 0.1 * jj
...
@jit(nogil=True, nopython=True)
def acc_get_density_cut_twographextra(
    check_list,
    merged_data,
    adj,
    density_record,
    ...
    for ii in check_list:
        ...
        for jj in check_list:
            ...
            if adjunct_label:
                ...
                continue
            adj[Ne] = (
                merged_id1 * Nnode + merged_id2
            )  # Redundant, every possible connect save twice
            density_record[Ne] = min(
                merged_data[ii, 4], merged_data[jj, 4]
            )  # Once one don't exist, then the connection will be not exist by this specific ori points
            Ne += 1  # count for effective edges
    return adj, Ne, density_record


@jit(nogil=True, nopython=True)
def acc_get_density_cut_twograph(
    Nori,
    merged_data,
    adj,
    density_record,
