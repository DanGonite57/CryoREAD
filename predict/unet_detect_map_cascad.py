    ...
    for x in range(0, scan_x, stride):
        ...
        for y in range(0, scan_y, stride):
            ...
            for z in range(0, scan_z, stride):
                ...
                ] = map_data[x_start:x_end, y_start:y_end, z_start:z_end]
                if contour <= 0:
...
def make_predictions(
    ...
    Base_Matrix = np.zeros(
        [base_classes, overall_shape[0], overall_shape[1], overall_shape[2]]
    )
...
def unet_detect_map_cascad(
    ...
    if os.path.exists(coord_path):
        ...
        Coord_Voxel = gen_input_data(
            map_data, voxel_size, stride, contour, train_save_path
        )
    ...
    base_class = 5
    model = Cascade_Unet(
        in_channels=1,  # include density and probability array
    ...
    if os.path.exists(cur_prob_path) and os.path.exists(cur_label_path):
        ...
        Prediction_Matrix, Prediction_Label, Base_Matrix, Base_Label = make_predictions(
            test_loader,
            model,
            Coord_Voxel,
            voxel_size,
