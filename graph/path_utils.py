...
def collect_all_searched_path(
    subgraphs, pho_point, pho_graph, search_dir, map_info_list, params
):
    ...
    for cluster_id, subgraph in enumerate(subgraphs):
        ...
        mkdir(subgraph_path)
        pho_coordinate_list, pho_edge_pairs = Pick_Graph_coord(
            subgraph,
        ...
        listfiles = [
            x for x in os.listdir(subgraph_path) if ".cif" in x and "path" in x
        ]
        listfiles.sort()
        if len(listfiles) < 1:
            ortools_build_path(
            ...
            listfiles.sort()
        Path_ID_List = Prune_Selected_Path(
            subgraph_path,
            listfiles,
            pho_point.merged_cd_dens,
            pho_graph,
