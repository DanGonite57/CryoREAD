...
def merge_cluster_cif_PS(
    ...
    if len(current_cluster_id_list) == 1:
        ...
        current_ldp_location = ldp_sugar_location[
            starting_index : starting_index + cur_ldp_size
        ]
        current_pho_ldp_location = Path_P_align_list[
            starting_index : starting_index + cur_ldp_size
        ]
        if cur_direction == -1:
            # adjust interval and write from end to begin
            cur_interval = [
                cur_chain_length - cur_interval[1] - 1,
                cur_chain_length - cur_interval[0],
            ]  # reverse chain to correct direction
            match_seq = match_seq[::-1]
            current_ldp_location = current_ldp_location[::-1]
            current_pho_ldp_location = Path_P_reverse_align_list[
                starting_index : starting_index + cur_ldp_size
            ]
        ...
        current_pho_ldp_location = [
            pho_merged_cd[int(kk)] for kk in current_pho_ldp_location
        ]
    ...
    assert verify_chain_check(
        current_path_dict, current_cluster_id_list
    )  # assure same chain, same direction, same chain_length
    current_seq_info = current_path_dict[current_cluster_id_list[0]]
    ...
    for k in range(len(score_sort)):
        ...
        cur_ldp_size = len(
            cur_match_seq
        )  # in case it's short for some near tail regions
        overall_match_seq = (
            overall_match_seq[:shift_pos]
            + cur_match_seq
            + overall_match_seq[(shift_pos + cur_ldp_size) :]
        )
    ...
    if overall_direction == -1:
        ...
        overall_residue_id_list = [
            overall_chain_length - x - 1 if x != -1 else -1
            for x in overall_residue_id_list
        ]
    ...
    for k in range(len(overall_match_seq)):
        if overall_match_seq[k] == "-":
            ...
            cur_residue = (
                overall_residue_id_list[k] + 1
            )  # the original starting from 0, not good
            # make sure assignment in ['A','U','C','G']
            assert overall_match_seq[k] in ["A", "U", "C", "G", "T"]
            overall_score += cur_score
            assert cur_residue != -1
            if (
                current_pho_ldp_align_info[k] >= 0
            ):  # avoid non-assign regions always give a wrong point
                fragment_info_list.append(
                    [
                        overall_chain,
                        cur_residue,
    ...
    for path_id in current_cluster_id_list:
        ...
        if cur_direction == -1:
            # adjust interval and write from end to begin
            cur_interval = [
                cur_chain_length - cur_interval[0] - cur_useful_size,
                cur_chain_length - cur_interval[0],
            ]  # reverse chain to correct direction
            match_seq = match_seq[::-1]
            shift_pos = (
                end_index - begin_index - shift_pos - cur_ldp_size
            )  # prev it's the distance to the ending part, now we swapped
        current_seq_index = cur_interval[0]
        cur_score = current_seq_info["score"]
        align_length = len([x for x in match_seq if x != "-"])
        avg_score = cur_score / align_length
...
def clean_pho_assign_info_list(
    frag_info_list, pho_coordinate, map_info_list, cutoff=7, round=1
):
    ...
    if len(Pho_Position_List) > 0:
        ...
        for i in range(len(frag_info_list)):
            ...
            if (
                cur_sp_distance1[close_location_index] > cutoff
                or cur_sp_distance2[close_location_index] > cutoff
            ):
                continue
            ...
            print(
                "previous loc ", cur_atom_position, "updated loc: ", select_pho_location
            )
            final_frag_info_list.append(cur_info)
        return final_frag_info_list, True
    print(
        "duplicate use of %d/%d pho positions in atomic"
        % (len(Remove_SeqID_set), len(Sugar_ID_list))
    )
    if round == 1:
        ...
        print(
            "revise %d/%d pho positions in atomic"
            % (len(Remove_SeqID_set), len(Sugar_ID_list))
        )
        ...
        for pair in remove_pair_list:
            ...
            cur_sp_distance2 = distance_array[sugar_index]
            prev_sp_distance = (
                cur_sp_distance1[prev_pho_index] + cur_sp_distance2[prev_pho_index]
            )
            sugar_index = int(Sugar_IDdefine_dict[int(follow_seqid)])
            cur_sp_distance1 = distance_array[sugar_index - 1]
            cur_sp_distance2 = distance_array[sugar_index]
            follow_sp_distance = (
                cur_sp_distance1[follow_pho_index] + cur_sp_distance2[follow_pho_index]
            )
        ...
        for i in range(len(frag_info_list)):
            ...
            if current_seq_index not in Remove_SeqID_set:
                ...
                continue
        print(
            "clearning changed %d to %d frag info"
            % (len(frag_info_list), len(final_frag_info_list))
        )
