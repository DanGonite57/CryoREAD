...
def global_align_score(
    ...
    for i in range(len(ldp_distance_array)):
        if i == 0:
            ...
            # 1 choose
            prev_neighbor = (
                int(ldp_prev_connect[i - 1]) if ldp_prev_connect[i - 1] != -1 else i - 1
            )
            ...
            if (
                ...
                current_best = (
                    scratch[i - 1]
                    + match_matrix[i]
                    - ldp_gap_penalty * abs(cur_distance - mean_distance) ** 2
                )  # ldp_gap
            else:
...
def dynamic_assign_geo(
    updated_base_list, ldp_gap_penalty, fragment_distance_array, save_path
):
    ...
    scratch, pointer, ldp_prev_connect = global_align_score(
        ldp_gap_penalty,
        match_matrix,
        pointer,
        scratch,
...
def greedy_assign_geo(
    ...
    for path_id, cur_path_list in enumerate(All_Base_Path_List_sugar):
        ...
        max_score, match_seq_line = dynamic_assign_geo(
            study_ldp_base_list, ldp_gap_penalty, fragment_distance_array, dp_save_path
        )
        dp_save_path = os.path.join(cur_path_save_path, "path_reverse_order")
        mkdir(dp_save_path)
        max_score2, match_seq_line2 = dynamic_assign_geo(
            study_ldp_base_list[::-1],
            ldp_gap_penalty,
            fragment_distance_array[::-1, ::-1],
            dp_save_path,
        )
        new_dict = {}
