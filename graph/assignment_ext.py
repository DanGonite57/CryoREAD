...
def reorganize_missing_list(missing_list, define_ldp_size):
    ...
    print(
        "reformed missing list length %d, previous %d"
        % (len(final_missing_list), len(missing_list))
    )
    return final_missing_list


def Extend_Solve_Assignment_SP_support(
    save_path,
    All_Path_List_sugar,
    ...
    for order_index in solve_frag_combine_list:
        ...
        occupy_dict[ldp_path1].append(
            [
                ldp_starting_index1,
                ldp_starting_index1 + len(current_seq_info1["match_seq"]),
            ]
        )
    ...
    for ldp_path_id in range(len(All_Path_List_sugar)):
        ...
        if len(identified_ldp_frag_list) == 0:
            ...
            continue
        identified_ldp_frag_list = sorted(
            identified_ldp_frag_list, key=(lambda x: x[0])
        )
        for k in range(len(identified_ldp_frag_list) - 1):
            if k == 0:
                ...
                if next_starting_index - cur_ending_index <= gap_limit:
                    continue
                current_match_info = current_path_match_info[
                    k
                ]  # check which index is the last matched residue, then we use that as a starting referece
                cur_last_real_match_idx = (
                    identify_last_match(current_match_info["match_seq"])
                    + cur_starting_index
                )
                Missing_Range_Dict[ldp_path_id].append(
                    [cur_last_real_match_idx, next_starting_index]
                )
        ...
        if total_ldp_list_length - final_ending_frag_index > gap_limit:
            ...
            cur_last_real_match_idx = (
                identify_last_match(current_match_info["match_seq"])
                + identified_ldp_frag_list[-1][0]
            )
            Missing_Range_Dict[ldp_path_id].append(
                [cur_last_real_match_idx, total_ldp_list_length]
            )
    ...
    for ldp_path_id in range(len(All_Path_List_sugar)):
        ...
        missing_list = reorganize_missing_list(
            missing_list, define_ldp_size
        )  # specifically for strict since some missing region is even bigger compared to
        for k in range(len(missing_list)):
            ...
            current_seq_info = overall_dict[current_key][
                0
            ]  # use top 1 as match candidate
            print("before info:", current_seq_info)
            remove_match_seq = current_seq_info["match_seq"][
                (fill_ending_index - fill_starting_index) :
            ]
            current_seq_info["match_seq"] = current_seq_info["match_seq"][
                : fill_ending_index - fill_starting_index
            ]

            count_remove_match_num = len(
                [1 for tmp_match_id in remove_match_seq if tmp_match_id != "-"]
            )

            current_seq_info["interval"][1] = (
                current_seq_info["interval"][1] - count_remove_match_num
            )
