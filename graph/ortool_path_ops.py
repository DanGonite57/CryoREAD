...
def create_ortool_data_model(
    ...
    max_Value = 99999999
    adj_matrix = (
        np.ones([len(coordinate_list) + 1, len(coordinate_list) + 1]) * max_Value
    )  # super big to avoid this connections
    ...
    adj_matrix[:, 0] = 0
    adj_matrix[
        np.arange(1, len(coordinate_list) + 1), np.arange(1, len(coordinate_list) + 1)
    ] = 0
            ...
            for j in range(1, len(coordinate_list) + 1):
                if adj_matrix[i, j] == max_Value:
                    ...
                    if cur_distance >= cutoff_length:
                        adj_matrix[i, j] = int(
                            2 * max_distance + cur_distance * 200
                        )  # big penalty for them
                    else:
...
def ortools_build_path(
    ...
    if number_vehicles <= 5:
        number_vehicles = 5
    ortool_data = create_ortool_data_model(
        coordinate_list,
        edge_pairs,
        edge_density,
        number_vehicles,
        cutoff_length,
        relax_choice=relax_choice,
    )
    ...
    drop_penalty = int(
        2 * 100 * np.max(edge_density) + cutoff_length * 200
    )  # int(np.max(distance_matrix[distance_matrix!=max_Value]))
    print("drop penalty %d" % drop_penalty)
    # Create the routing index manager.
    manager = pywrapcp.RoutingIndexManager(
        len(ortool_data["distance_matrix"]),
        ortool_data["num_vehicles"],
        ortool_data["depot"],
    )
    ...
    search_parameters.time_limit.seconds = max(
        3600, int(1000 * len(coordinate_list) / 400)
    )  # few hours search
