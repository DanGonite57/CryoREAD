...
def reassign_basedonfrag(
    ...
    cur_frag_path = os.path.join(
        save_dir,
        "Fragment_reassign_%d_%d_%d.pkl" % (ldp_size, checking_stride, top_select),
    )
    ...
    old_frag_path = os.path.join(
        save_dir,
        "Fragment_assemble_%d_%d_%d.pkl" % (ldp_size, checking_stride, top_select),
    )
    ...
    for ldp_path_id in Path_Assign_Dict.keys():
        ...
        for cluster_id in interval_clusters:
            ...
            if len(current_cluster_id_list) == 1:
                ...
                Path_ReAssign_Dict[ldp_path_id][begin_index] = current_path_dict[
                    begin_index
                ]
                continue
            ...
            for k in range(len(current_cluster_id_list)):
                cur_ldp_size = len(
                    current_path_dict[current_cluster_id_list[k]]["match_seq"]
                )
            ...
            fragment_ldp_location = cur_ldp_sugar_location[
                start_ldp_index:end_ldp_index
            ]
            ...
            end_seq_index = min(
                overall_chain_length, int(init_interval[1] + ldp_size / 2)
            )
            current_chain_sequence = current_chain_sequence[
                begin_seq_index:end_seq_index
            ]
            ...
            dp_save_path = os.path.join(
                save_dir,
                "path%d_starting%d_chain_%s"
                % (ldp_path_id, start_ldp_index, overall_chain),
            )
            mkdir(dp_save_path)
            ...
            new_dict["interval"] = [
                begin_seq_index + current_match_seq_interval[0],
                begin_seq_index + current_match_seq_interval[1],
            ]
...
def merge_assign_geo_seq(overall_geo_dict, overall_seq_dict):
    ...
    for ldp_path_id in overall_geo_dict.keys():
        ...
        all_starting_index_keys = [
            int(x) for x in current_seqpath_dict.keys()
        ]  # should already be in order
        all_seq_assign_length = [
            len(current_seqpath_dict[tmp_path_id]["match_seq"])
            for tmp_path_id in current_seqpath_dict
        ]
        assert len(all_starting_index_keys) == len(np.unique(all_starting_index_keys))
        all_starting_index_keys = np.array(all_starting_index_keys)
        all_seq_assign_length = np.array(all_seq_assign_length)
        ...
        for starting_index in all_starting_index_keys:
            ...
            current_match_seq = (
                current_match_seq[:starting_index]
                + seqassign_match_seq
                + current_match_seq[starting_index + len(seqassign_match_seq) :]
            )
            print(
                "reassign %d index of %s to %s"
                % (starting_index, prev_seq, current_match_seq)
            )
