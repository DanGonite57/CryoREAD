...
from graph.LDP_ops import (
    build_LDP,
    Build_Base_LDP,
    Build_Baseall_LDP,
    prepare_all_sugar_location,
)
...
from graph.Assign_ops import (
    Assign_Base_Main_Path_sugar,
    Assign_PhoLDP_Sugarpath,
    Assign_Base_Main_Path,
)
...
def refine_structure(
    ...
    try:
        ...
        Gen_MaskDRNA_map(
            chain_prob, origin_map_path, mask_map_path, params["contour"], threshold=0.6
        )
        ...
        if params["colab"]:
            os.system(
                "cd %s;"
                " /content/phenix/phenix-1.20.1-4487/build/bin/phenix.real_space_refine"
                ' %s %s resolution=%.4f output.suffix="_phenix_refine"'
            ...
            os.system(
                "cd %s; phenix.real_space_refine %s %s resolution=%.4f"
                ' output.suffix="_phenix_refine" skip_map_model_overlap_check=True'
                % (
                    frag_collect_dir,
                    format_pdb_path,
                    mask_map_path,
        ...
        while not os.path.exists(gen_pdb_path) and count_check < 5:
            gen_pdb_path = format_pdb_path[:-4] + "_phenix_refine_00%d.pdb" % (
                count_check + 1
            )
            count_check += 1
        if not os.path.exists(gen_pdb_path):
            print(
                "please check final non-refined atomic structure in %s"
                % refine0_pdb_path
            )
            return
        try:
            ...
            if params["colab"]:
                ...
                coot_software = "coot"
            coot_refine_structure(
                refine1_pdb_path, mask_map_path, refine2_pdb_path, coot_software
            )
            ...
            while not os.path.exists(phenix_final_pdb) and count_check < 5:
                phenix_final_pdb = refine2_pdb_path[:-4] + "_phenix_refine_00%d.pdb" % (
                    count_check + 1
                )
                count_check += 1
            if os.path.exists(phenix_final_pdb):
                ...
                print(
                    "please check final refined atomic structure in %s"
                    % refine3_pdb_path
                )
                print("You can also check other refined output here %s" % output_dir)
            else:
                print(
                    "please check final refined atomic structure (pdb format) in this"
            ...
            if os.path.exists(gen_pdb_path):
                ...
                print(
                    "please check final refined atomic structure in this directory %s"
                    % frag_collect_dir
                )
...
def Build_Unet_Graph(
    ...
    map_data, mapc, mapr, maps, origin, nxstart, nystart, nzstart = process_map_data(
        origin_map_path
    )
    ...
    if (
        fasta_path is not None
        and os.path.exists(fasta_path)
        and os.path.getsize(fasta_path) > 0
    ):
    ...
    Gen_MaskProtein_map(
        chain_prob, origin_map_path, mask_map_path, params["contour"], threshold=0.3
    )
    ...
    input_mrc.upsampling_pho_prob(
        pho_prob, threshold=pho_prob_threshold, filter_array=None
    )
    input_mrc.upsampling_sugar_prob(
        sugar_prob, threshold=sugar_prob_threshold, filter_array=None
    )
    ...
    sugar_point = build_LDP(
        input_mrc,
        input_mrc.sugar_dens,
        input_mrc.sugar_Nact,
        origin_map_path,
        sugar_point_path,
        "sugar",
        params,
    ...
    sugar_graph, sugar_coordinate_list, sugar_edge_pairs, sugar_edge_d_dens = (
        construct_graph(
            input_mrc,
            input_mrc.sugar_dens,
            sugar_point,
            ...
            map_info_list,
            params,
            prob_threshold=sugar_prob_threshold,
            extend=True,
        )
    )
    ...
    All_Path_List = collect_all_searched_path(
        subgraphs, sugar_point, sugar_graph, sugar_search_path, map_info_list, params
    )
    ...
    All_Base_Path_List_sugar = Assign_Base_Main_Path_sugar(
        All_Path_List,
    ...
    Visualize_Path_Base(
        os.path.join(save_path, "base_assignsugar_combine"),
        All_Path_List,
    ...
    if not os.path.exists(base_refer_path):
        sugar_base_match_dict = Match_Sugar_Base_Location(
            All_Path_List, sugar_point, Base_LDP_List, map_info_list
        )
    ...
    Path_P_align_list, Path_P_reverse_align_list, Path_Pho_ID_List = (
        Assign_PhoLDP_Sugarpath(All_Path_List, sugar_point, pho_point)
    )
    # visualize the pho pdbs
    for k, path in enumerate(Path_Pho_ID_List):
        Visualize_LDP_Path(
            os.path.join(save_path, "support_p_%d.cif" % k),
            "support_p_%d" % k,
            path,
    ...
    Pho_Prob_Refer_List, Pho_Prob_Refer_Dict = Assign_Base_Main_Path(
        Path_Pho_ID_List,
        pho_point.merged_cd_dens[:, :3],
        Base_LDP_List,
        chain_prob[2:6],
    ...
    Pho_Prob_Refer_List_Reverse, Pho_Prob_Refer_Reverse_Dict = Assign_Base_Main_Path(
        Path_Pho_ID_List,
        pho_point.merged_cd_dens[:, :3],
        Base_LDP_List,
        chain_prob[2:6],
    ...
    overall_dict, frag_location_dict = greedy_assign_geo(
        All_Base_Path_List_sugar,
        All_Path_List,
        Path_P_align_list,
    ...
    sugar_path_based_location = prepare_all_sugar_location(
        All_Path_List, sugar_point, map_info_list
    )
    ...
    Build_Atomic_Structure(
        overall_dict,
        sugar_path_based_location,
        frag_collect_dir,
        Path_P_align_list,
        Path_P_reverse_align_list,
        pho_point,
        map_info_list,
        sugar_base_match_dict,
    )
    ...
    if params["no_seqinfo"] or chain_dict is None:
        if chain_dict is None:
            print(
                "!!!parsing fasta input failed, we can not output structures"
                " considering sequence assignment!!!"
            )
        ...
        if params["refine"]:
            ...
            print(
                "please check final output atomic structure in %s" % nonrefined_pdb_path
            )
        return
    else:
    ...
    if chain_dict is not None:
        if params["thread"] == 1:
            overall_dict, frag_location_dict = greedy_assign_PS(
                All_Base_Path_List_sugar,
                All_Path_List,
                Path_P_align_list,
                Path_P_reverse_align_list,
            ...
            overall_dict, frag_location_dict = greedy_assign_PS_effective(
                All_Base_Path_List_sugar,
                All_Path_List,
                Path_P_align_list,
                Path_P_reverse_align_list,
        ...
        print(
            "!!!parsing fasta input failed, we can not output structures considering"
            " sequence assignment!!!"
        )
        exit()
    ...
    mkdir(frag_save_path)
    cur_final_assemble_path = os.path.join(
        frag_save_path,
        "assemble_frag_%d_%d_%d_soft%d.txt"
        % (ldp_size, checking_stride, top_select, params["rule_soft"]),
    )

    cur_collision_path = os.path.join(
        frag_save_path,
        "Collision_Table_%d_%d_%d_soft_%d.npy"
        % (ldp_size, checking_stride, top_select, params["rule_soft"]),
    )
    collision_table, order_key_index, order_chain_index, key_order_index = (
    ...
    if not os.path.exists(cur_final_assemble_path):
        solve_frag_combine_list = solve_assignment(
            collision_table, order_key_index, order_chain_index, overall_dict
        )
    ...
    frag_collect_dir = os.path.join(
        frag_save_path,
        "atomic_reassign_%d_%d_%d" % (ldp_size, checking_stride, top_select),
    )
    mkdir(frag_collect_dir)
    overall_reassign_dict = reassign_basedonfrag(
    ...
    mkdir(frag_collect_dir)
    Extra_Added_Assign_Dict = Extend_Solve_Assignment_SP_support(
        frag_save_path,
        All_Path_List,
        solve_frag_combine_list,
        order_key_index,
        order_chain_index,
        overall_dict,
        define_ldp_size=ldp_size,
    )
    print(
        "We added %d assignment that have collision to fill those gaps"
        % (len(Extra_Added_Assign_Dict))
    )
    Build_Atomic_Model_nonoverlap_frag(
        overall_reassign_dict,
        sugar_path_based_location,
        ldp_size,
        frag_collect_dir,
        checking_stride,
    ...
    init_pdb_path = os.path.join(
        frag_collect_dir,
        "Final_Assemble_%d_%d_%d.pdb" % (ldp_size, checking_stride, top_select),
    )
    format_pdb_path = os.path.join(
        frag_collect_dir,
        "Final_Assemble_%d_%d_%d_formated.pdb"
        % (ldp_size, checking_stride, top_select),
    )
    format_pdb(init_pdb_path, format_pdb_path, DNA_Label)
    output_dir = os.path.join(root_save_path, "Output")
    mkdir(output_dir)
