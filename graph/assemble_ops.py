...
def build_collision_table(
    ...
    collision_table = np.zeros([N_fragement, N_fragement]).astype(
        int
    )  # 1 denotes forbidding co-existance
        ...
        for j in range(1, N_fragement):
            ...
            if ldp_path1 == ldp_path2:  # if they are in the same path tracing
                ...
                if ldp_union_ending_index - ldp_union_starting_index < 2 * ldp_size:
                    ...
                    ldp_overlap = True
                    ldp_intersection_start = max(
                        ldp_starting_index1, ldp_starting_index2
                    )
                    ...
                    if (
                        match_intersect_direction1 != match_intersect_direction2
                        or match_intersect_chain1 != match_intersect_chain2
                    ):
                    ...
                    if not bool_intersection(
                        match_intersect_interval1, match_intersect_interval2
                    ):
                        collision_table[i, j] = 1
                        collision_table[j, i] = 1
                    ...
                    match_align1_begin = (
                        ldp_intersection_start - ldp_union_starting_index
                    )
                    match_align1_useful_count = len(
                        [
                            k
                            for k in match_intersect_seq1[:match_align1_begin]
                            if k != "-"
                        ]
                    )
                    # calculate the sequence assignment of match 1 aligned interval
                    match_align1_interval = [
                        match_intersect_interval1[0] + match_align1_useful_count,
                        match_intersect_interval1[1],
                    ]
                    match_align1_seq = match_intersect_seq1[match_align1_begin:]

                    match_align2_end = ldp_intersection_end - ldp_intersection_start
                    match_align2_useful_count = len(
                        [k for k in match_intersect_seq2[:match_align2_end] if k != "-"]
                    )
                    match_align2_interval = [
                        match_intersect_interval2[0],
                        match_intersect_interval2[0] + match_align2_useful_count,
                    ]
                    ...
                    if not bool_intersection(
                        match_align1_interval, match_align2_interval
                    ):
                        collision_table[i, j] = 1
                        collision_table[j, i] = 1
                    ...
                    cur_identity = calculate_identity(
                        match_align1_seq, match_align2_seq
                    )
            ...
            if ldp_overlap is False and soft_rule == 0:
                ...
                match_intersect_interval1 = match_info1["interval"]
                match_intersect_interval2 = match_info2["interval"]
                match_seqfrag1_length = abs(
                    match_intersect_interval1[1] - match_intersect_interval1[0]
                )
                match_seqfrag2_length = abs(
                    match_intersect_interval2[1] - match_intersect_interval2[0]
                )
                ...
                if match_intersect_direction1 != match_intersect_direction2:
                    match_chain1_new_interval = [
                        match_intersect_chain1length - match_intersect_interval1[1] - 1,
                        match_intersect_chain1length - match_intersect_interval1[0],
                    ]
                    if (
                        overlap_size(
                            match_chain1_new_interval, match_intersect_interval2
                        )
                        >= max_seqfrag_length * overlap_cutoff
                    ):
                        collision_table[i, j] = 1
                        collision_table[j, i] = 1
    ...
    print(
        "3. nearby assignment did not satisfy distance constraint:%d"
        % count_seq_collision3
    )
...
def solve_assignment(collision_table, order_key_index, order_chain_index, overall_dict):
    ...
    for i in range(len(collision_table)):
        model.Add(
            sum(x[j] for j in range(len(collision_table)) if collision_table[i][j] == 1)
            == 0
        ).OnlyEnforceIf(x[i])

    score_array = prepare_score_array(
        len(collision_table), order_key_index, order_chain_index, overall_dict
    )
