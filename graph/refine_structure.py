...
def refine_structure(input_pdb, input_map, output_dir, params):
    ...
    if "resolution" not in params:
        params["resolution"] = 5
        print(
            "no resolutino input detected, use 5A as default resolution for refinement!"
        )
    os.system(
        "cd %s; phenix.real_space_refine %s %s resolution=%.4f "
        'output.suffix="_phenix_refine" '
        "skip_map_model_overlap_check=True"
        % (output_dir, format_pdb_path, input_map, params["resolution"])
    )
    ...
    os.system(
        ...
        'output.suffix="_phenix_refine" '
        "skip_map_model_overlap_check=True"
        % (output_dir, refine2_pdb_path, input_map, params["resolution"])
    )
