    ...
    for i in range(1, iterator1, 1):
        ...
        for j in range(1, iterator2, 1):
            for k in range(1, iterator3, 1):
                count = [
                    int(i / prev_voxel_size),
                    int(j / prev_voxel_size),
                    int(k / prev_voxel_size),
                ]
                e1 = count[0] + 1
                e2 = count[1] + 1
                e3 = count[2] + 1
                ...
                if (
                    count[0] >= size[0] - 1
                ):  # or count[1]>=size[1]-1 or count[2]>=size[2]-1 ):
                    # print(count)
                    e1 = count[0]
                    continue
                if count[1] >= size[1] - 1:
                ...
                if count[2] >= size[2] - 1:
                    ...
                    continue
                diff1 = [
                    i - count[0] * prev_voxel_size,
                    j - count[1] * prev_voxel_size,
                    k - count[2] * prev_voxel_size,
                ]
                diff2 = [
                    e1 * prev_voxel_size - i,
                    e2 * prev_voxel_size - j,
                    e3 * prev_voxel_size - k,
                ]
                # print(diff)
                ...
                val = (
                    (
                        w2
                        * (v1 * (u1 * val3 + u2 * val4) + v2 * (u1 * val2 + u2 * val1))
                        + w1
                        * (v1 * (u1 * val7 + u2 * val8) + v2 * (u1 * val6 + u2 * val5))
                    )
                    / (w1 + w2)
                    / (v1 + v2)
                    / (u1 + u2)
...
def interpolate_fast_general(
    ...
    for i in range(1, iterator1, 1):
        ...
        for j in range(1, iterator2, 1):
            for k in range(1, iterator3, 1):
                count = [
                    int(i / prev_voxel_size1),
                    int(j / prev_voxel_size2),
                    int(k / prev_voxel_size3),
                ]
                e1 = count[0] + 1
                e2 = count[1] + 1
                e3 = count[2] + 1
                ...
                if count[2] >= size[2] - 1:
                    ...
                    continue
                diff1 = [
                    i - count[0] * prev_voxel_size1,
                    j - count[1] * prev_voxel_size2,
                    k - count[2] * prev_voxel_size3,
                ]
                diff2 = [
                    e1 * prev_voxel_size1 - i,
                    e2 * prev_voxel_size2 - j,
                    e3 * prev_voxel_size3 - k,
                ]
                # print(diff)
                val1 = data[count[0], count[1], count[2]]
                val2 = data[e1, count[1], count[2]]
...
def Reform_Map_Voxel_Final(map_path, new_map_path):
    ...
    if not os.path.exists(new_map_path):
        with mrcfile.open(map_path, permissive=True) as mrc:
            ...
            for i in range(
                it_val_x
            ):  # progressbar(range(it_val_x), prefix="", size=60):
                # if i%10==0:
...
def Reform_Map_Voxel(map_path, new_map_path):
    if not os.path.exists(new_map_path):
        with mrcfile.open(map_path, permissive=True) as mrc:
            ...
            if not (
                prev_voxel_size["x"] == prev_voxel_size["y"]
                and prev_voxel_size["x"] == prev_voxel_size["z"]
            ):
                print(
                    "Grid size of different axis is different, please specify"
                    " --resize=1 in command line to call another slow process to deal"
                    " with it!"
                )
                exit(1)
            ...
            if prev_voxel_size < 1:
                print(
                    "Grid size is smaller than 1, please specify --resize=1 in command"
                    " line to call another slow process to deal with it!"
                )
                exit(1)
            ...
            print(
                "Previouse size:", size, " Current map size:", it_val1, it_val2, it_val3
            )
...
if __name__ == "__main__":
    ...
    with ProgressBar(total=it_val1) as progress:
        interpolate_fast(
            data, data_new, size, it_val1, it_val2, it_val3, prev_voxel_size, progress
        )
    with ProgressBar(total=it_val1) as progress:
        data_new2 = interpolate_slow(
            data, data_new, size, it_val1, it_val2, it_val3, prev_voxel_size, progress
        )
    # from ops.progressbar import progressbar
    it_val_x = it_val1
